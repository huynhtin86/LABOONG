...
function AppContent() {
  ...
  const [settings, setSettings] = useState<Settings>({ dailyRevenueTarget: 1000000, monthlyRevenueTarget: 30000000, adminEmails: [] });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#f5f5f0] p-4">
        <div className="max-w-md w-full bg-white rounded-3xl p-8 shadow-xl text-center">
          ...
          <h1 className="text-3xl font-serif font-bold text-[#1a1a1a] mb-2">Laboong Manager</h1>
          <p className="text-gray-600 mb-8">Quản lý tồn kho và doanh thu quán trà sữa của bạn một cách chuyên nghiệp.</p>
          <button 
            onClick={handleLogin}
            className="w-full bg-[#5A5A40] text-white py-4 rounded-full font-medium hover:bg-[#4a4a35] transition-colors flex items-center justify-center gap-2"
          >
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f0] flex flex-col md:flex-row">
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto">
        ...
        {activeTab === 'settings' && isAdmin && <SettingsView settings={settings} />}
...
function SettingsView({ settings }: { settings: Settings }) {
  ...
  const performReset = async () => {
    ...
    try {
      ...
      let totalDeleted = 0;
      let summary = "Kết quả xóa dữ liệu:\n";
      ...
      for (const colName of collectionsToClear) {
        try {
          ...
          for (let i = 0; i < docs.length; i += 500) {
            ...
            console.log(`Deleted batch of ${chunk.length} from ${colName}. Total so far: ${totalDeleted}`);
          }
          summary += `- ${colName}: ${colDeleted} bản ghi\n`;
        } catch (colError) {
          console.error(`Error clearing ${colName}:`, colError);
          summary += `- ${colName}: LỖI (${colError instanceof Error ? colError.message : 'Không xác định'})\n`;
        }
      }
      
      console.log("Data reset complete. Total items deleted:", totalDeleted);
      ...
      alert(`Có lỗi nghiêm trọng xảy ra: ${error instanceof Error ? error.message : 'Lỗi không xác định'}`);
    } finally {
  ...
  return (
    <div className="space-y-8 max-w-2xl">
      <header>
        <h2 className="text-3xl font-serif font-bold text-[#1a1a1a]">Cài đặt</h2>
        <p className="text-gray-500">Cấu hình mục tiêu doanh thu và hệ thống.</p>
      </header>

      <div className="bg-white p-8 rounded-3xl shadow-sm border border-black/5 space-y-6">
        {showSuccess && (
          <div className="bg-emerald-100 text-emerald-700 p-4 rounded-2xl font-bold animate-in fade-in slide-in-from-top-2">
            Đã lưu cài đặt thành công!
          </div>
        )}
        <div className="space-y-2">
          <label className="text-sm font-bold text-gray-600">Mục tiêu doanh thu ngày (VNĐ)</label>
          <input 
            type="number" 
            className="w-full p-4 rounded-2xl border border-gray-200 text-xl font-bold"
            value={tempSettings.dailyRevenueTarget}
            onChange={e => setTempSettings({...tempSettings, dailyRevenueTarget: Number(e.target.value)})}
          />
        </div>
        <div className="space-y-2">
          <label className="text-sm font-bold text-gray-600">Mục tiêu doanh thu tháng (VNĐ)</label>
          <input 
            type="number" 
            className="w-full p-4 rounded-2xl border border-gray-200 text-xl font-bold"
            value={tempSettings.monthlyRevenueTarget}
            onChange={e => setTempSettings({...tempSettings, monthlyRevenueTarget: Number(e.target.value)})}
          />
        </div>
        ...
        <div className="pt-6 border-t border-gray-100 space-y-6">
          <h3 className="font-bold text-lg flex items-center gap-2">
            ...
            Ràng buộc vị trí chấm công
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="text-sm font-bold text-gray-600">Vĩ độ (Latitude)</label>
              <input 
                type="number" 
                step="any"
            ...
            <div className="space-y-2">
              <label className="text-sm font-bold text-gray-600">Kinh độ (Longitude)</label>
              <input 
                type="number" 
                step="any"
                className="w-full p-4 rounded-2xl border border-gray-200"
            ...
            <div className="space-y-2">
              <label className="text-sm font-bold text-gray-600">Bán kính cho phép (mét)</label>
              <input 
                type="number" 
                className="w-full p-4 rounded-2xl border border-gray-200"
        ...
        <div className="pt-6 border-t border-gray-100 space-y-4">
          <h3 className="font-bold text-lg flex items-center gap-2">
            <Wifi className="w-5 h-5 text-[#5A5A40]" />
            Ràng buộc WiFi (Tùy chọn)
          </h3>
          <div className="space-y-2">
            <label className="text-sm font-bold text-gray-600">Tên WiFi cho phép (SSID)</label>
            <input 
              type="text" 
              className="w-full p-4 rounded-2xl border border-gray-200"
              value={tempSettings.allowedWifi || ''}
              onChange={e => setTempSettings({...tempSettings, allowedWifi: e.target.value})}
              placeholder="VD: MilkTea_Shop_Guest"
            />
            <p className="text-xs text-gray-400 italic">* Lưu ý: Trình duyệt web không thể tự động kiểm tra SSID vì lý do bảo mật. Thông tin này dùng để đối chiếu thủ công hoặc hiển thị nhắc nhở.</p>
          </div>
        </div>
        <div className="pt-6 border-t border-gray-100 space-y-4">
          <h3 className="font-bold text-lg flex items-center gap-2">
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {tempSettings.adminEmails?.map(email => (
                <div key={email} className="bg-gray-100 px-4 py-2 rounded-full flex items-center gap-2 text-sm font-medium">
                  {email}
                  <button 
            ...
            <p className="text-xs text-gray-400 italic">* Các email trong danh sách này sẽ có toàn quyền quản trị hệ thống khi đăng nhập.</p>
          </div>
        </div>
        <button 
          onClick={handleSave}
        ...
        <div className="pt-10 border-t border-red-100">
          <div className="bg-red-50 p-6 rounded-3xl border border-red-100 space-y-4">
            ...
            <p className="text-sm text-red-600">Xóa toàn bộ dữ liệu hệ thống để bắt đầu lại từ đầu. Hành động này không thể hoàn tác.</p>
            
            {resetStep === 0 && (
              <button 
                onClick={() => setResetStep(1)}
                disabled={isResetting}
                className="w-full bg-red-600 text-white py-4 rounded-2xl font-bold shadow-lg hover:bg-red-700 transition-all flex items-center justify-center gap-2"
              >
                <Trash2 className="w-5 h-5" />
                Xóa hết dữ liệu & Reset hệ thống
              </button>
            )}

            {resetStep === 1 && (
              <div className="bg-white p-4 rounded-2xl border-2 border-red-200 animate-in zoom-in-95 duration-200">
                <p className="text-red-700 font-bold mb-4 text-center">Bạn có chắc chắn muốn xóa TOÀN BỘ dữ liệu?</p>
            ...
            {resetStep === 2 && (
              <div className="bg-white p-4 rounded-2xl border-2 border-red-500 animate-in zoom-in-95 duration-200">
                <p className="text-red-700 font-bold mb-4 text-center">XÁC NHẬN CUỐI CÙNG: Hành động này không thể hoàn tác!</p>
                <div className="grid grid-cols-2 gap-3">
                  <button 
                    onClick={() => setResetStep(0)}
                    className="bg-gray-100 text-gray-700 py-3 rounded-xl font-bold"
            ...
            {isResetting && resetStep !== 3 && (
              <div className="flex flex-col items-center justify-center py-4 gap-3">
                <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-red-600"></div>
                <p className="text-red-600 font-bold">Đang xóa dữ liệu hệ thống...</p>
              </div>
            )}
          </div>
        </div>
